...
(function (module) {
    ...
    function sync(client, db, logger, listURL, remoteVersionByID) {
        logger.info("replicate sync", client.id, listURL, remoteVersionByID);
    ...
    function store(client, db, logger, url, timestamp, modifications) {
        logger.info("replicate store", client.id, url, timestamp, modifications);
        ...
        return cb(undefined, {
            ...
            "replicate":function (command, url) {
                switch (command) {
                    ...
                    case "store":
                        return store(this, db, logger, url, arguments[2], arguments[3]);
                }
